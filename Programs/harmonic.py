#!/usr/bin/python2
"""\
 harmonic.py Solve the eigenvectors and eigenvalues of the harmonic
  oscillator. The problem solved here is described in
...
def OutputSolutions(ham):
    ...
    alpha = 10 #scaling factor for output
    for i in range(n):
        x = ham.x(i)
        file.write("%5.3f %5.3f %5.3f %5.3f %5.3f \n" %\
                   (x,50*x*x,
                    eval[0]+alpha*evec[0,i],
                    eval[1]+alpha*evec[1,i],
                    eval[2]+alpha*evec[2,i]))
    return

def main(n):
    "The program starts here."
    # RPM: danger: there is a problem here with npts>21. Fix!!
    ham = oned_ham(n,-3.,3.,lambda x: 50*x*x)
    ham.getHam()
